#!/usr/bin/env bash
# =============================================================================
# SoundMaster — 静的ファイルを FTP でレンタルサーバーへアップロード
# =============================================================================
#
# 【使い方】
#   推奨: cp deploy.env.example deploy.env を編集（deploy.env は .gitignore）
#   ./deploy-ftp.sh           ← 差分転送
#   ./deploy-ftp.sh --dry-run ← 確認のみ（転送しない）
#
# 【フロー】
#   npm run build で dist/ を生成
#   → リモートに soundmaster/ があれば soundmaster.NNN へリネーム（バックアップ）
#   → ローカルの dist/ 内容をリモートへミラー転送
#
# 【依存】
...
# --- 転送対象（ビルド成果物） -------------------------------------------------
UPLOAD_DIR="dist"
UPLOAD_FILES=(index.html assets/ favicon.svg)
...
使い方: ./deploy-ftp.sh [オプション]
  ...
  SoundMaster の静的ファイルを FTP でリモートへアップロードします。
...
FTP_USER="${FTP_USER:-your_account}"
FTP_PASS="${FTP_PASS:-your_password}"
FTP_REMOTE_PATH="${FTP_REMOTE_PATH:-/public_html/soundmaster}"
...
if is_placeholder_config; then
  ...
  echo "  cp deploy.env.example deploy.env を編集してください。" >&2
  exit 1
fi
...
for f in "${UPLOAD_FILES[@]}"; do
  ...
  [[ ! -f "${SCRIPT_DIR}/${UPLOAD_DIR}/${f}" ]] && MISSING+=("${UPLOAD_DIR}/${f}")
done
if [[ ${#MISSING[@]} -gt 0 ]]; then
  echo "エラー: 転送対象ファイルが見つかりません: ${MISSING[*]}" >&2
  echo "  先に npm run build を実行してください。" >&2
  exit 1
fi
