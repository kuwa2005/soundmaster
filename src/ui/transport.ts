...
export function renderTransport(container: HTMLElement) {
  ...
  container.innerHTML = `
    <div class="transport-row flex items-center gap-4">
      <div class="flex items-center gap-2">
        <button id="btn-play" class="transport-btn daw-btn daw-btn-primary px-5 py-2 font-medium ${!hasTrack ? 'opacity-50 cursor-not-allowed' : ''}" ${!hasTrack ? 'disabled' : ''}>
          ${state.isPlaying ? t('transport.pause') : t('transport.play')}
        </button>
      ...
      <div class="ab-toggle flex items-center gap-3">
      ...
      <div id="time-display" class="time-display font-mono text-sm tabular-nums" style="color: var(--color-daw-muted); background: var(--color-daw-panel); padding: 4px 12px; border-radius: 4px;">
        00:00.000
      </div>
    </div>
  `
